...
public final class ZupixServer implements AutoCloseable {
    ...
    public static ZupixServer create(int port, Router router, MiddlewareRegistry middleware, ExceptionRegistry exceptions) throws IOException { Objects.requireNonNull(router,"router"); HttpServer server=HttpServer.create(new InetSocketAddress(port),0); ZupixServer runtime=new ZupixServer(server,router,middleware,exceptions); server.createContext("/",runtime::handle); server.setExecutor(Executors.newVirtualThreadPerTaskExecutor()); return runtime; }
