...
public final class MiddlewarePipeline {
    ...
    public MiddlewarePipeline(List<Middleware> middlewares) { this.middlewares = List.copyOf(middlewares); }

    public void execute(RequestContext request, Runnable endpoint) { invoke(0, request, endpoint); }

    private void invoke(int index, RequestContext request, Runnable endpoint) {
        if (index >= middlewares.size()) { endpoint.run(); return; }
